...
import type TopScorersDTO from '../models/DTO/TopScorersDTO'
...
export const TopScorers = (): React.JSX.Element => {
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [visiblePageCount, setVisiblePageCount] = useState<number>(10);

    const { isPending: loading, data: playerStats = [], error } = useQuery<TopScorersDTO[]>({
        queryKey: ['playerStats'],
        queryFn: async () => {
            const response = await axios.get('/players/top-scorers');
    ...
    if (error) {
        return <div>Unable to load top scorers.</div>
    }
                        ...
                        <>
                            <thead>
                                <tr>
                                    ...
                                    <th>Average Rebounds Per Game</th>
                                    <th>Average Assists Per Game</th>
                                </tr>
                            </thead>
                            <tbody>
