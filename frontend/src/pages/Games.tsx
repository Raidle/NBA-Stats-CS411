...
import type TeamGameStatsDTO from '../models/DTO/TeamGameStatsDTO'
...
export const Games = (): React.JSX.Element => {
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [visiblePageCount, setVisiblePageCount] = useState<number>(10);

    const { isPending: loading, data: teamStats = [], error } = useQuery<TeamGameStatsDTO[]>({
        queryKey: ['gameStats'],
        queryFn: async () => {
            const response = await axios.get('/teams/home-away');
    ...
    const pageSize = 10;
    const totalPages = Math.max(1, Math.ceil(teamStats.length / pageSize));
    ...
    const paginatedTeamStats = useMemo(() => {
        ...
        return teamStats.slice(startIndex, startIndex + pageSize);
    ...
    if (error) {
        return <div>Unable to load team game stats.</div>
    }
                        ...
                        <>
                            <thead>
                                <tr>
                                    <th>Team Name</th>
                                    <th>City</th>
                                    <th>Average Home Score</th>
                                    <th>Average Away Score</th>
                                </tr>
                            </thead>
                            <tbody>
                                {paginatedTeamStats.map((team) => (
                                    <tr key={team.teamId}>
                                        <td>{team.teamName}</td>
                                        <td>{team.city}</td>
                                        <td>{team.avgHomeScore}</td>
                                        <td>{team.avgAwayScore}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </>
